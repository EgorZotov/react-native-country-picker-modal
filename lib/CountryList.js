...
const CountryItem = (props) => {
    ...
    const { country, onSelect, withFlag, withEmoji, withCallingCode, withCurrency, customStyles } = props;
    const extraContent = [];
    if (withCallingCode &&
        country.callingCode &&
...
export const CountryList = (props) => {
    const { data, withAlphaFilter, withEmoji, withFlag, withCallingCode, withCurrency, onSelect, filter, flatListProps, filterFocus, customStyles } = props;
    ...
    return (React.createElement(View, { style: [styles.container, { backgroundColor }] },
        React.createElement(FlatList, Object.assign({ ref: flatListRef, testID: 'list-countries', keyboardShouldPersistTaps: 'handled', automaticallyAdjustContentInsets: false, scrollEventThrottle: 1, getItemLayout: (_data, index) => ({
